...
def run_test(sess, model, batch_gen, data):
    ...
    cont=0
    if IS_LOGGING:
        while(os.path.exists('analysis/audio.txt'+str(cont))==True and os.path.exists('analysis/audio_label.txt'+str(cont))== True):
            cont=cont+1
        with open( '../analysis/audio.txt'+str(cont), 'w' ) as f:
            f.write( ' '.join( [str(x) for x in list_pred] ) )

        with open( '../analysis/audio_label.txt'+str(cont), 'w' ) as f:
